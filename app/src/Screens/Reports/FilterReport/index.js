...
const Reports = (props) => {
  ...
  function handleNavigateToReportPosition(positions, vehicleSelectedParam) {
    props.navigation.navigate('ReportPosition', {
      positions,
      vehicleSelected: vehicleSelectedParam,
    });
  }
  ...
  function prepareInfo(info) {
    return `Motorista :  ${info.motorista || 'Não informado'}
Endereço :  ${info.address}
Data posição :  ${info.data}
Data comunicação :  ${info.data_comunica}
Bloqueado : ${info.bloqueado === 'N' ? 'NÃO' : 'SIM'}
Ligado :  ${info.ligado === 'N' ? 'NÃO' : 'SIM'}
Bateria :  ${info.voltagem_bateria}
RPM :  ${info.rpm || 'Não informado'}
Evento :  ${info.evento || 'Não informado'}
Velocidade :  ${info.velocidade} KM/h`;
  }
  ...
  async function handleGenerateReport() {
    try {
      ...
      form.append('speed', vehicleSelected.speed);
      ...
      data.forEach((p, index) => {
        if (index !== data.length - 1) {
          const rotation = Math.floor(
            bearing(p.lat, p.lng, data[index + 1].lat, data[index + 1].lng),
          );
      ...
      handleNavigateToReportPosition(coordinates, vehicleSelected);
    } catch (error) {
            ...
            {vehicleSelected?.id ? (
              <S.VehicleSelected>
                ...
                <S.VehicleSelectedButtonDelete
                  onPress={() => setVehicleSelected(null)}
                >
