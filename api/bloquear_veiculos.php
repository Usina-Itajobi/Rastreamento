<?php
	...
	*/
	...
	$stm = mysqli_query($con,"select * from comandos where cmd_modelo_rastreador = '" . $modelo_rastreador . "' and cmd_block = '1'" );
	$n = mysqli_num_rows ( $stm );
	
	if( $n == 0){
		$retorno = array( 'msg' => 'Comando nao cadastrado para o rastreador ('. $modelo_rastreador .') do imei(' . $imei . '). Entrar em contato com o administrador do sistema. id-cli = ' . $id_cliente . ' - ' . $_POST['v_imei']);
		echo json_encode( $retorno );
		die;
	}
	...
	$retorno = array( 'msg' => 'Sinal de bloquear enviado com sucesso!');
	echo json_encode( $retorno );
	die;
