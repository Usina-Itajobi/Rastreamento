<?php
	...
	*/
	...
	$idbem = protejeInject ( $_POST['id'] );
	if(!$idbem){
		$idbem = protejeInject ( $_GET['id'] );
	}

	$qual = protejeInject ( $_POST['qual'] );
	if(!$qual){
		$qual = protejeInject ( $_GET['qual'] );
	}

	
	$sql_up = "update bem set alert_ign = '". $qual ."' where id = '". $idbem ."'";
	mysqli_query($con,$sql_up);

	$retorno = array( 'msg' => 'alerte ligado enviado com sucesso!');
	echo json_encode( $retorno );
	die;
