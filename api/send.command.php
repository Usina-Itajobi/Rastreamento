...
if($id_cliente != 2231){
	...
	$imei = $_POST['imei'];
	if( $imei == '' ){
		$retorno = array('errormsg'=>'Preencha o Imei.' , 'error' => 'S' );
		echo json_encode( $retorno );
		die;
	}
	...
	if( $command == ''  ){
		$retorno = array('errormsg'=>'Preencha o comando.' , 'error' => 'S' );
		echo json_encode( $retorno );
		die;
	}
	
	if( $command != 'block' && $command != 'unblock' ){
		$retorno = array('errormsg'=>'Preencha o comando correto.' , 'error' => 'S' );
		echo json_encode( $retorno );
		die;
	}
	
	if( $command == 'block' ){
		 $sqlcommand = "select * from comandos where cmd_modelo_rastreador = '" . $modelo_rastreador . "' and cmd_block = '1'" ;
		 $qsinal = 'bloquear';
	}elseif( $command == 'unblock' ){
		 $sqlcommand = "select * from comandos where cmd_modelo_rastreador = '" . $modelo_rastreador . "' and cmd_unblock = '1'" ;
		  $qsinal = 'desbloquear';
	}
	
	$stm = mysqli_query($con, $sqlcommand);
	$n = mysqli_num_rows ( $stm );
	
	if( $n == 0){
		$retorno = array( 'error' => 'S', 'errormsg' => 'Comando nao cadastrado para o rastreador ('. $modelo_rastreador .') do imei(' . $imei . '). Entrar em contato com o administrador do sistema. id-cli = ' . $id_cliente . ' - ' . $_POST['imei']);
		echo json_encode( $retorno );
		die;
	}
	...
	$retorno = array( 'msg' => 'Sinal de '. $qsinal .' enviado com sucesso!');
	echo json_encode( $retorno );
	die;

}else{$retorno = array( 'msg' => 'erro sem permissao!');
	echo json_encode( $retorno );
	die;}